...
#define TARGET_DELAY  30.0
#define TARGET_BLOCK  2.0
        ...
        for (int L = 1; L <= 15; L++){
                ...
                m.avg_delay     <= MAX_AVG_DELAY)
            {
                if (best_Ng == -1) {
                    ...
                    best_tie_score = fabs(m.delay_prob - TARGET_DELAY)
                                   + fabs(m.blocking_prob - TARGET_BLOCK)
                    ...
                    if (Ng < best_Ng) {
                        ...
                        double score = fabs(m.delay_prob - TARGET_DELAY)
                                     + fabs(m.blocking_prob - TARGET_BLOCK)
                                     + fabs(m.avg_delay - TARGET_AVG_DELAY);
