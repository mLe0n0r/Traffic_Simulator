...
#define MAX_HIST_ERROR 60.0  // Erro máximo esperado (ex: 60s)
#define NUM_BINS 30          // Número de 'caixas' no histograma
...
EC_Metrics call_center_system(double lambda, int N_general, int N_specific, int L)
{
    ...
    while (arrivals < num_events){
        ...
        if(nextG <= nextS){
            ...
            handle_general_call(&events_list, &specific_list, &queue, N_general, L, &busy, &delayed, &blocked, &in_queue, &out_of_queue, &sum_delay, ev_type, ev_purpose, ev_time, arrivals, &running_avg, &sum_abs_error, &sum_rel_error, &prediction, results.histogram, results.num_bins, results.bin_width);
        }
        else{
...
int main(void){
    ...
    if (best_Ng >= 0){
        ...
        srand(time(NULL)); 
        EC_Metrics final_run = call_center_system(lambda, best_Ng, best_Ns, best_L);

        if (final_run.histogram != NULL) {
            for (int i = 0; i < final_run.num_bins; i++) {
